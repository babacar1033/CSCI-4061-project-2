...

{
  ...
  //declare a variabl msg of type struct msgBuffer to represent the chunk4
...
void shuffle(int nMappers, int nReducers) {
  ...
  for (int i =0; i< nMappers; i+=1)
  {
    ...
    DIR* dir = opendir("output/MapOut/Map_mapperID");
  ...
  while(entry = readdir(dir)) // traverse the directory of 1 mapper
  {
    ...
    {
     int reducerId = hashFunction(entry->d_name , nReducers); //selecting the reducer using the already defined hash function.
